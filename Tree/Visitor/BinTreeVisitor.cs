...
namespace BinaryTreeProj.Tree {
    abstract class BinTreeVisitor {
        ...
        public IEnumerable<T> visit<T>(BinaryNode<T> tree) where T : INodeType<T> {
            List<T> buffer = new List<T>();
            getAllNodesRec(tree, ref buffer);
            return buffer;
        }

        abstract protected void getAllNodesRec<T>(BinaryNode<T> tree, ref List<T> buffer) where T : INodeType<T>;
    }
}